...
Package body Command_Analyser is
 use command_def;
   ...
   function Turnout_preference(My: in Train_state; Turnout: in raildefs.Turnout_Id) return raildefs.turnout_pos is
   use raildefs;
   ...
   procedure Assign_Ahead(My: in out Train_state;
   ...
   procedure Resume(My: in out Train_state;
                    Me: in Raildefs.Train_Id) is
   ...
   procedure Free_blocks(My: in out Train_state) is
   use raildefs; --for /=
unassign_block: Block_Id;
   begin
      ...
      procedure Turn_around (My: in out Train_state;
                          Me: in Raildefs.Train_Id ) is
...
use Ada.Calendar;
   ...
   procedure Front_analyses (My: in out Train_State_Def.Train_state;
                             Me: in Raildefs.Train_Id)is
 use topolog2;
...
--      Ada.Text_IO.Put_line(my.Speed'img);
      ...
      if( Alerted) then
       --  Ada.Text_IO.Put_line("entereing zone");
         ...
         My.Max_speed:=Train_State_Def.Zone_Max_speed;--crawl up to boundry
   ...
   procedure Back_analyses(My: in out  Train_State_Def.Train_state; Me: in Raildefs.Train_Id) is
 use topolog2;
       ...
       -- Ada.Text_IO.Put_line("not leaving turout");
         ...
         if(alerted) then
           -- Ada.Text_IO.Put_line("leaving zone");
            Rail_element.Unassign_Zone(zone_alert);
         my.Max_speed:= Train_State_Def.Abs_max_speed;
   ...
   procedure Check_Waiting(My: in out Train_State_Def.Train_state; Me: in Raildefs.Train_Id) is
...
task type User_Input_anlyseis(Me: Raildefs.train_id) is
      pragma Priority (10) ;
   end User_Input_anlyseis;

   User1: User_input_anlyseis(1);
   User2: User_input_anlyseis(2);
   User3: User_input_anlyseis(3);

task body User_Input_anlyseis is
 use raildefs; --needed for /=
 use Topolog2; --needed for =
   user_input: User_input_type;
   Buffer_error:Boolean;
   user_input_command:Command_Type;
   begin
      loop
         Train_state_array(Me).User_input_buffer.Remove(user_input_command,Buffer_error);
         if(Buffer_error) then Ada.text_io.Put_Line("User input buffer Error"); end if;

	user_input:= user_input_command.Input.input_type;
         case user_input is
         when Speed =>
            if (user_input_command.Input.Speed_increase = speed_up)then
               if Train_state_array(Me).Speed < Train_state_array(Me).Max_speed then
                  Train_state_array(Me).Speed := Train_state_array(Me).Speed + 10;
               end if;

               if Train_state_array(Me).Pre_zone_speed <Train_State_Def.Abs_max_speed then
                  Train_state_array(Me).Pre_zone_speed :=Train_state_array(Me).Pre_zone_speed+10;
               end if;

            else --must be a slow down
               if Train_state_array(Me).Speed > 0 then
                  Train_state_array(Me).Speed := Train_state_array(Me).Speed - 10;
               end if;

               if Train_state_array(Me).Pre_zone_speed > 0 then
                  Train_state_array(Me).Pre_zone_speed := Train_state_array(Me).Pre_zone_speed - 10;
               end if;
            end if;

            if(
               (Train_state_array(Me).Turnout_waiting=No_Turnout)  and then
                 (Train_state_array(Me).Chain_Turnout_waiting=No_Turnout)  and then
                 (Train_state_array(Me).Crossing_waiting=No_Crossing) and then
                 (Train_state_array(Me).Block_waiting=No_Block) and then
                 (Train_state_array(Me).Zone_waiting=No_zone)
              ) then
               Dac_driver.Set_Voltage(Cab_Type(Me), Train_state_array(Me).Speed);

               if(Train_state_array(Me).Speed>0 ) then
                  if Train_state_array(Me).Moving=false then
                     Resume(Train_state_array(Me),me);
                  end if;
               else
                  Train_state_array(Me).Moving:=false;
                  Train_state_array(Me).Block_waiting:= raildefs.No_Block;
                  Train_state_array(Me). Turnout_waiting:= raildefs.No_Turnout;
                  Train_state_array(Me). Chain_Turnout_waiting:= raildefs.No_Turnout;
                  Train_state_array(Me). Crossing_waiting:= raildefs.No_Crossing;
                  Train_state_array(Me). Chain_Crossing_waiting:= raildefs.No_Crossing;
                  Train_state_array(Me). Zone_waiting:= topolog2.No_Zone;
               end if;
         ...
         when Polarity =>
            if( user_input_command.Input.Polarity_update /= Train_state_array(Me).Relative_Direction) then
               Train_state_array(Me).Relative_Direction := user_input_command.Input.Polarity_update;
               Turn_around(Train_state_array(Me), Me);

            end if;


         when Path =>
            Train_state_array(Me).Path_preference := user_input_command.Input.Path_preference;
         end case;
      end loop;
   end User_Input_anlyseis;


   procedure Analyse(Me: in Raildefs.Train_Id; My: in out Train_state; Command: in Command_Type)is
   ...
   begin--Analyse
      ...
      case Command.Eve is
         when User_Input =>
            My.User_input_buffer.insert(command);
         when hall_event=>
