...
Command_Buffer_array : array (Raildefs.Train_Id) of Buffers.Command_Buffer;
      ...
      Train_state_array:array(Raildefs.Train_Id) of train_state_dec.Train_state;
 	...
 	if(Error)then
      ...
      if Me = 0 then
         ...
         when User_Input =>
            ...
            --for testing /\ /\
         Command_Analyser.Analyse(Me,Train_state_array(Me),Command);
   ...
   --Ada.Text_IO.Put_line("sorting");
      ...
      for T in Train_Id loop
         ...
         if (Command.Sensor = Train_state_array(T).Front_sensor.Next_Expected or else Command.Sensor = Train_state_array(T).Back_sensor.Next_Expected) then
...
use Raildefs; -- for compare
   ...
   begin--Init
      case Train_id is
      when 1=>
         Train_state_array(Train_id).owned_Blocks.insert(2,Buffer_error);
         if(Buffer_error) then Ada.text_io.Put_Line("Block Buffer Error"); end if;

         Train_state_array(Train_id).Front_sensor:=Topolog2.Initialise_After(3);
         Train_state_array(Train_id).Back_sensor:=Topolog2.Initialise_After(3);
      when 2 =>
         Train_state_array(Train_id).owned_Blocks.insert(12,Buffer_error);
         if(Buffer_error) then Ada.text_io.Put_Line("Block Buffer Error"); end if;
	 ...
	 if(Train_state_array(Train_id).Speed>0) then
         Train_state_array(Train_id).Moving:= true;
      else
      Train_state_array(Train_id).Moving:= false;
            end if;
      if(Rail_element.Assign_Block(Train_id,Train_state_array(Train_id).owned_Blocks.get_Front,
                                   Train_state_array(Train_id).Absalute_Direction)) then
         null;
      end if;
       if(Rail_element.Assign_Block(Train_id,Train_state_array(Train_id).owned_Blocks.get_Last ,
                                   Train_state_array(Train_id).Absalute_Direction)) then
         null;
         end if;
